    ...
    addKeyboardShortcuts() {
      ...
      return {
        ...
        'ArrowLeft': () => {
          ...
          if (cursor.depth() > 0) {console.log(cursor.depth()); this.editor.commands.selectParentNode(); this.editor.commands.scrollIntoView(); return true} else {this.editor.commands.focus('start', { scrollIntoView: true }); return true}
        },
        // ArrowRight moves the position of the cursor to the next position, which can be the next text position or the next node.
        'ArrowRight': () => {this.editor.commands.focus(cursor.pos() + 1, { scrollIntoView: true }); return true},
        ...
        'ArrowDown': () => {
          ...
          else {this.editor.commands.focus(cursor.pos()+cursor.nextNodeSize(),{ scrollIntoView: true }); return true}
        },
        ...
        'ArrowUp': () => {
          if (cursor.inTextNode() || (cursor.prevNodeSize() === 0 && cursor.nextNodeSize() === 0)) {return false}
          else {this.editor.commands.focus(cursor.pos()-cursor.prevNodeSize(),{ scrollIntoView: true }); return true}
        },
